...
void loop(){
  ...
  if(canMsg.can_id == 0xAAA){
    //Motor(채널, 방향, 조절하고 싶은 모터의 속도)
      if(canMsg.data[0] == fast_mode){ // 초음파가 15미만이 아니고 wii 값이 30초과이고 50이 아니면 모터 속도 15
          Motor(1,1,15);          // 50이 아닌 조건을 넣은 이유는 모터 가속도를 값을 추출하기위해 추가하였음  
         // Motor(2,1,15, 1);
      }
      else if(canMsg.data[0] == average_mode){  // 가속도를 위해 wii 값이 50이면 모터 속도 30
         Motor(1,1,30);
          //Motor(2,1,30, 1);
    }
      else if(canMsg.data[0] == slow_mode){  // 초음파 값이 15미만이면 메시지 값으로 0을 받고 0이면 모터 정지
          Motor(1,0,0);
          //Motor(2,0,0, 1);
      }
